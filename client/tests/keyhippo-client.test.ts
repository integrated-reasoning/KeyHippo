...
describe("KeyHippo Client Tests", () => {
  ...
  it("should handle creating an API key with a duplicate description", async () => {
    ...
    const firstKey = await testSetup.keyHippo.createApiKey(
      testSetup.userId,
      keyDescription,
    );
    ...
    const secondKey = await testSetup.keyHippo.createApiKey(
      testSetup.userId,
      keyDescription,
    );
  ...
  it("should not return revoked API keys in key info", async () => {
    const keyDescription = "Key to be Revoked";
    ...
    // Load API key info and ensure revoked key is not present
    const keyInfos = await testSetup.keyHippo.loadApiKeyInfo(testSetup.userId);
    const revokedKey = keyInfos.find((key) => key.id === createdKeyInfo.id);
  ...
  it("should retrieve the correct API key info after rotation", async () => {
    const keyDescription = "Key to Rotate";
    ...
    const newKeyExists = keyInfos.some((key) => key.id === rotatedKeyInfo.id);
    expect(newKeyExists).toBe(true);
  });

  it("should prevent SQL injection when modifying an API key description", async () => {
    // Create two API keys, one to attack and one to modify
    const validDescription = "Valid Test Key";
    const attackDescription =
      "Attack Key'; UPDATE keyhippo.api_key_id_name SET name = 'Attacked';--";

    // Create the first valid API key
    const validKeyInfo = await testSetup.keyHippo.createApiKey(
      testSetup.userId,
      validDescription,
    );
